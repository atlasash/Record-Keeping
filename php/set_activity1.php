...
$dbname = DB_NAME;
	...
	$study = $_POST['set_activity1_study'];
	$director = $_POST['set_activity1_director'];
	...
	if($director == 1){
		...
		while ($row = mysql_fetch_array($result, MYSQL_NUM)) {
			echo "<script type='text/javascript'>parent.setActivity1List('" . $row[0] . "','" . $row[0] . "');</script>";
		}

	}
	else{
		$result = mysql_query("SELECT activity_1 FROM activity_level_1 WHERE type='" . $study_type . "' AND Director='0'");
		while ($row = mysql_fetch_array($result, MYSQL_NUM)) {
			echo "<script type='text/javascript'>parent.setActivity1List('" . $row[0] . "','" . $row[0] . "');</script>";
		}
	}
