...
$dbname = DB_NAME;
	...
	$dbuser = $username;
	$dbpass = $password;
	$user = $_POST['reject_timeoff_user'];
	$ID = $_POST['reject_timeoff_id'];
...
/************************   Get Requested Days/Hours START   *************/
		...
		$result = mysql_query("SELECT hours,start_date,end_date,type,user_login,rejected FROM request WHERE ID='" . $ID . "' LIMIT 1");
		...
		if($rejected == "1"){
			echo "<script type='text/javascript'>alert('Request has already been rejected.');</script>";
		...
		if($request_type == 'Vacation'){
			...
			if($days_requested > $days_available){
				...
				mysql_connect($host, $username, $password) or
					die("Could not connect: " . mysql_error());
				mysql_select_db($dbname);
				$result = mysql_query("UPDATE request SET approved='0',rejected='1' WHERE ID='" . $ID . "'");
				mysql_free_result($result);

				if($user == $_POST['reject_timeoff_user'])
				{
